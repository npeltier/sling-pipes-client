...
var mapping = {
  ...
  it("parseSubCommand should parse a subcommand and return a JSONObject", function() {
    ...
    expect(Pipes.Creation.parseSubCommand("rm   /content/foo", mapping,1)).toEqual({
        "jcr:primaryType":"nt:unstructured",
        "sling:resourceType":"slingPipes/rm",
        "path":"/content/foo"
    });
    expect(Pipes.Creation.parseSubCommand("rm", mapping,1)).toEqual({
        "jcr:primaryType":"nt:unstructured",
        "sling:resourceType":"slingPipes/rm"
    });
    expect(Pipes.Creation.parseSubCommand("$  /content/foo  cq:Page", mapping,1)).toEqual({
        "jcr:primaryType":"nt:unstructured",
        "sling:resourceType":"slingPipes/slingQuery",
        "path":"/content/foo",
        "expr":"cq:Page"
    });
    expect(Pipes.Creation.parseSubCommand("base",mapping,1)).toEqual({
        "jcr:primaryType":"nt:unstructured",
         "sling:resourceType":"slingPipes/base"
    });
    expect(Pipes.Creation.parseSubCommand("write sling:resourceType=blah",mapping,1)).toEqual({
          "jcr:primaryType":"nt:unstructured",
